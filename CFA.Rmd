...
## Analisis Factorial Exploratorio (Clásico)
...
```{r}
mot_model <- 'AP =~ ap_1 + ap_2 + ap_3 + ap_4 + ap_5 + ap_6 + ap_7 + ap_8
CE =~ ce_1 + ce_2 + ce_3 + ce_4 + ce_5 + ce_6 + ce_7 + ce_8 + ce_9
EE =~ ee_1 + ee_2 + ee_3 + ee_4 + ee_5 + ee_6 + ee_7 + ee_8
AE =~ ae_1 + ae_2 + ae_3 + ae_4 + ae_5 + ae_6 + ae_7 + ae_8'
fitMot <- lavaan::cfa(mot_model, data = T1, ordered = names(T1))
```


```{r}
semPaths(fitMot, what = "std", edge.label.cex = 0.7,
edge.color = 1, esize = 1, sizeMan = 4.5, asize = 2.5, intercepts = FALSE, rotation = 4, thresholdColor = "red", mar = c(1, 5, 1.5, 5), fade = FALSE, nCharNodes = 4)
```

Varianza del error de los indicadores:
...
Cargas y sus correspondientes versiones estandarizadas:
...
Matriz de covarianza de la variable latente:

```{r}
inspect(fitMot, what = "est")$psi
inspect(fitMot, what = "std")$psi #standardized version (correlation matrix)
```

Las pruebas expl??citas de si las cargas (y otros par??metros del modelo) difieren de 0, incluidos los IC del 95%, se pueden calcular a trav??s de:

```{r}
parameterEstimates(fitMot, standardized = TRUE)
```

Las pruebas de significancia se llevan a cabo sobre los par??metros no estandarizados. El argumento standardized = TRUE agrega las columnas std.lv y std.all, indicando estimaciones estandarizadas cuando solo las variables latentes est??n estandarizadas, y estimaciones estandarizadas cuando las variables latentes y observadas est??n estandarizadas, respectivamente.


Para obtener el resultado completo del modelo, incluidas las medidas de bondad de ajuste:
