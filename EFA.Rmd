...
# Modelo EFA (de 1 a 5 factores)

```{r}
FA <- fa(cor$rho, nfactors = 1, rotate = "none", fm = "ml")
print(FA$loadings)
```

```{r}
round(FA$communality, 2)
```


```{r}
FA <- fa(cor$rho, nfactors = 2, rotate = "none", fm = "ml")
print(FA$loadings)
```
...
```{r}
FA <- fa(cor$rho, nfactors = 3, rotate = "none", fm = "ml")
print(FA$loadings)
```
...
```{r}
FA <- fa(cor$rho, nfactors = 4, rotate = "none", fm = "ml")
print(FA$loadings)
```
...
```{r}
FA <- fa(cor$rho, nfactors = 5, rotate = "none", fm = "ml")
print(FA$loadings)
```
...
```{r}
FA <- fa(cor$rho, nfactors = 6, rotate = "none", fm = "ml")
print(FA$loadings)
```

```{r}
FA <- fa(cor$rho, nfactors = 7, rotate = "none", fm = "ml")
print(FA$loadings)
```
...
```{r}
FA <- fa(cor$rho, nfactors = 4, rotate = "varimax", fm = "ml")
print(FA$loadings)
```


# Determinar el número de factores: Metodo codo

```{r}
cor_rho <- polychoric(T1)$rho 
evals <- eigen(cor_rho)$values 
scree(cor_rho, factors = FALSE)
```


```{r}
(evals/sum(evals)*100)[1:10]
```


# Determinar el número de factores: Metodo parallel analisis

```{r}
set.seed(123)
resPA <- fa.parallel(T1, fa = "pc", cor = "poly",fm = "ml")
```


# Determinar el número de factores: Metodo VSS

```{r}
vss(T1, fm = "ml", n.obs = nrow(T1), plot = FALSE)
```
...
```{r}
resnf <- nfactors(T1, n = 4, fm = "ml", cor = "poly") 
resnf
```
